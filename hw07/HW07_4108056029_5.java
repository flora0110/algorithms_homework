public class HW07_4108056029_5 extends Buy_Phone{
    int n;
    int[][] ans;
    int[][] inputArr;
    int[][] aux;
    ...
    private void merge(int start, int mid, int end){
        ...
        inputArr[start++] = ((aux[i][0] < aux[j][0])||((aux[i][0] == aux[j][0])&&(aux[i][1] <aux[j][1])))? aux[i++] : aux[j++];
    ...
    private void Insertion(int from, int to){
        for(int i=from+1; i<to; i++){
            for(int j=i-1; j>=from && ((inputArr[j][0] > inputArr[j+1][0])||((inputArr[j][0] == inputArr[j+1][0])&&(inputArr[j][1] >inputArr[j+1][1]))); j--){
    ...
    public static void main(String[] args){
        HW07_4108056029_5 test = new HW07_4108056029_5();
